...
export function randomISI(): number {
  return Math.floor(Math.random() * (PVT_ISI_MAX - PVT_ISI_MIN + 1)) + PVT_ISI_MIN;
}

export function computeMetrics(trials: PVTTrial[], durationMs: number): PVTResult {
